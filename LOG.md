...
9. Las pruebas de inventario/paridad se actualizaron para que Π sea una capacidad activa, de acuerdo con la decisión de producto registrada por Track D.

## Verificación local

- `pytest tests/test_evaluate.py tests/test_parsing.py`: **85/85** después de los cambios.
- Casos centinela directos: `sum(i,i,1,5)=15`, `product(i,i,1,5)=120`, `50%=0.5`, `pm(5)={-5,5}`, rechazo de `foo(x)`/`dsolve(x)` y límite `1/x→0` marcado DNE.
- La validación frontend completa se ejecuta en GitHub Actions debido a que este entorno no pudo completar `npm ci` contra el registro npm.
