...
KNOWN = [
    ("sin(0)", 0.0), ("sin(pi/2)", 1.0), ("cos(0)", 1.0), ("cos(pi)", -1.0),
    ("tan(pi/4)", 1.0), ("sec(0)", 1.0), ("csc(pi/2)", 1.0), ("cot(pi/4)", 1.0),
    ("asin(1)", math.pi / 2), ("acos(1)", 0.0), ("atan(1)", math.pi / 4),
    ("sinh(0)", 0.0), ("cosh(0)", 1.0), ("tanh(0)", 0.0),
    ("asinh(0)", 0.0), ("acosh(1)", 0.0), ("atanh(0)", 0.0),
    ("asech(1)", 0.0), ("acsch(1)", math.asinh(1)), ("acoth(2)", 0.5 * math.log(3)),
]
...
def test_module01_domain_edges_are_controlled():
    failures = []
    for expression in [
        "tan(pi/2)", "sec(pi/2)", "csc(0)", "cot(0)",
        "atanh(1)", "asech(0)", "acoth(1)",
    ]:
        response = client.post("/api/v1/evaluate", json={"expression": expression, "angle_unit": "rad"})
        body = response.json()
        if not (
            ...
            and body.get("error_code") == "DOMAIN_ERROR"
        ):
            failures.append(
                f"{expression}: HTTP {response.status_code}, "
